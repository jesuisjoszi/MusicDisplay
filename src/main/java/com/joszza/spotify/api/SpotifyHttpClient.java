...
public final class SpotifyHttpClient {
    ...
    public static int putJson(@Nonnull String url, @Nonnull Map<String, String> headers, @Nonnull String jsonBody) {
        try {
            HttpURLConnection http = open(url, "PUT", headers);
            ...
            http.disconnect();
            if (code < 200 || code >= 300) {
                LOGGER.atWarning().log("Spotify PUT JSON %s failed: HTTP %s %s", url, code, response);
            }
            return code;
        } catch (Exception e) {
            LOGGER.atWarning().withCause(e).log("PUT JSON %s failed", url);
            return -1;
        }
    }
